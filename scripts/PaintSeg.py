...
if __name__ == "__main__":
    ...
    #print(f'{os.getcwd()}/{opt.dataset}')
    image_paths = sorted(glob.glob(f'{os.getcwd()}/{opt.dataset}/images/*'), reverse=True)
    print('image path',image_paths[0])
    if opt.split != 999:
        image_paths = image_paths[opt.split::2]

    save_path = image_paths[0].replace(f'{opt.dataset}', f'{opt.dataset}').replace('images', f'{opt.outdir}')
    ...
    for image_path in tqdm(image_paths):
        ...
        with torch.no_grad():
            ...
            #mask_path = os.path.join(f'{opt.dataset}/tokencut/', filename + '_tokencut.jpg')
            # mask_path = f'{opt.dataset}/tokencut/'+'sun_ekmqudbbrseiyiht.jpg'
            mask_path = os.path.join(f'{opt.dataset}/tokencut/', filename + '.png')

            if opt.gt:
                #mask_path = os.path.join(f'{opt.dataset}/tokencut/', filename + '_gt.jpg')
                mask_path = os.path.join(f'{opt.dataset}/tokencut/', filename + '.png')
            ...
            with model.ema_scope():
                ...
                save_path = image_path.replace(f'{opt.dataset}', f'{opt.dataset}').replace('images', f'{opt.outdir}')
